...
allowed-tools: Read, Write, Edit, Bash(cat:*), Bash(nvim:*), Bash(chezmoi:*)
---
...
- Source location: `dot_config/nvim/` (in chezmoi source directory)
- Target location: `~/.config/nvim/` (XDG standard)
- **Workflow:** Edit files in chezmoi source, user will run `chezmoi apply` to deploy

**Chezmoi naming:** The `dot_` prefix becomes `.` when applied (e.g., `dot_config` -> `.config`).

**Current state:** Fully configured with modular Lua-based setup

## Neovim Configuration Structure

### Current Structure (chezmoi-managed)
...
└── after/
    └── plugin/
        └── python-lsp.lua     # Python LSP overrides
```

**Note:** All paths shown are relative to chezmoi source (`~/.local/share/chezmoi/`).
...
Set in `dot_config/nvim/lua/config/options.lua`:
...
Set in `dot_config/nvim/lua/config/mappings.lua`:
...
Set in `dot_config/nvim/lua/config/lazy.lua`:
...
Set in `dot_config/nvim/lua/plugins/core.lua` (or dedicated LSP file):
...
Set in `dot_config/nvim/lua/config/autocmds.lua`:
...
1. **Edit files** in the chezmoi source directory (`dot_config/nvim/`)
2. **User applies changes** with `chezmoi apply` (do not run this automatically)
3. **Test** in Neovim
...
```
Read dot_config/nvim/init.lua
Read dot_config/nvim/lua/config/options.lua
Read dot_config/nvim/lua/config/mappings.lua
```
...
```lua
-- dot_config/nvim/lua/plugins/myplugin.lua
return {
  "author/plugin-name",
  dependencies = {},
  config = function()
...
**Append to mappings.lua:**
```lua
-- Add to dot_config/nvim/lua/config/mappings.lua
...
```lua
-- dot_config/nvim/lua/plugins/themes.lua
return {
  "catppuccin/nvim",
  name = "catppuccin",
  priority = 1000,
...
### 4. Add Language-Specific Config

For Python-specific configuration, use `dot_config/nvim/lua/plugins/python.lua` or override in `dot_config/nvim/after/plugin/`.
...
- Source: `dot_config/nvim/` (chezmoi-managed)
- Target: `~/.config/nvim/`
- Format: Lua-based with lazy.nvim
